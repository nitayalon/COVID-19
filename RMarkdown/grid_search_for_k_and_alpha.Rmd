...
italy_K_grid <- c(italy_grid_search_results_07_06_2020$nation_wide_rtt_results$inner_grid_search_results$K_CI[1], italy_grid_search_results_07_06_2020$nation_wide_rtt_results$inner_grid_search_results$profile_likelihood_K$k[which.max(italy_grid_search_results_07_06_2020$nation_wide_rtt_results$inner_grid_search_results$profile_likelihood_K$llk)], 
                  italy_grid_search_results_07_06_2020$nation_wide_rtt_results$inner_grid_search_results$K_CI[2])
italy_alpha_grid <- c(NA, italy_grid_search_results_07_06_2020$nation_wide_rtt_results$inner_grid_search_results$profile_likelihood_alpha$alpha[which.max(italy_grid_search_results_07_06_2020$nation_wide_rtt_results$inner_grid_search_results$profile_likelihood_alpha$llk)], NA)
italy_trajectories <- lapply(1:3, function(i){predictCovidTrajectory(italy_grid_search_results_07_06_2020$nation_wide_rtt_results$environment_data, italy_K_grid[i],italy_alpha_grid[i], alpha_grid = seq(0.5,0.7, length.out = 50))})
italy_alpha_grid <- c(italy_trajectories[[1]]$alpha, italy_alpha_grid[2], italy_trajectories[[3]]$alpha)
```

```{r plot Italy data}
plot(italy_grid_search_results_07_06_2020$nation_wide_rtt_results$inner_grid_search_results$profile_likelihood_alpha)
abline(h = -2.5, col = "red")
dates <- seq(as.Date('22/01/2020',format = '%d/%m/%y'),as.Date('01/08/2020',format = '%d/%m/%y'),'day')
last_date <- as.Date('01/09/2020',format = '%d/%m/%y')
state <- 'Italy'
debug(plotTrajectories)
plotTrajectories(italy_grid_search_results_07_06_2020$nation_wide_rtt_results$environment_data,
                 italy_grid_search_results_07_06_2020$covid_data_sets$transformed_covid_data[50:nrow(italy_grid_search_results_07_06_2020$covid_data_sets$transformed_covid_data),],
                 italy_trajectories,dates,last_date,'Italy',ylim = c(0,3),starting_day = 50, last_fitting_date = as.Date('25/05/2020',format = '%d/%m/%y'))
```
...
state <- 'Belgium'
belgium_grid_search_results <- mainFunction(state, 60, population_list[[state]], alpha_grid = seq(0.1,0.6,length.out = 40))
save(x = belgium_grid_search_results, file = sprintf('/home/nitay/COVID-19/R/Ad_hoc_computations/grid_search_results/%s.RData',state))

plot(belgium_grid_search_results$nation_wide_rtt_results$inner_grid_search_results$profile_likelihood_alpha)
plot(belgium_grid_search_results$nation_wide_rtt_results$inner_grid_search_results$profile_likelihood_K)
belgium_K_grid <- c(belgium_grid_search_results$nation_wide_rtt_results$inner_grid_search_results$K_CI[1], 
               belgium_grid_search_results$nation_wide_rtt_results$inner_grid_search_results$profile_likelihood_K$k[which.max(belgium_grid_search_results$nation_wide_rtt_results$inner_grid_search_results$profile_likelihood_K$llk)], 
               belgium_grid_search_results$nation_wide_rtt_results$inner_grid_search_results$K_CI[2])
belgium_alpha_grid <- c(NA, belgium_grid_search_results$nation_wide_rtt_results$inner_grid_search_results$profile_likelihood_alpha$alpha[which.max(belgium_grid_search_results$nation_wide_rtt_results$inner_grid_search_results$profile_likelihood_alpha$llk)], NA)
belgium_trajectories <- lapply(1:3, function(i){predictCovidTrajectory(belgium_grid_search_results$nation_wide_rtt_results$environment_data, belgium_K_grid[i],belgium_alpha_grid[i],alpha_grid = seq(0.1,0.8,length.out = 40))})
belgium_alpha_grid <- c(belgium_trajectories[[1]]$alpha, belgium_alpha_grid[2], belgium_trajectories[[3]]$alpha)
```
...
dates <- seq(as.Date('22/01/2020',format = '%d/%m/%y'),as.Date('01/11/2020',format = '%d/%m/%y'),'day')
last_date <- as.Date('01/11/2020',format = '%d/%m/%y')
plotTrajectories(belgium_grid_search_results$nation_wide_rtt_results$environment_data,
                 belgium_grid_search_results$covid_data_sets$transformed_covid_data[60:nrow(belgium_grid_search_results$covid_data_sets$transformed_covid_data),],
                 belgium_trajectories,dates,last_date,'belgium',ylim = c(0,0.8),starting_day = 60,scale_factor = 1e5)
```
