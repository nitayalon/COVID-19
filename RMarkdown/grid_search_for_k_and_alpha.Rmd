...
```{r Italy grid search}
state <- 'Italy'
italy_grid_search_results <- mainFunction(state,50, population_list[[state]])
...
italy_K_grid <- c(italy_grid_search_results$inner_grid_search_results$K_CI[1], italy_grid_search_results$inner_grid_search_results$profile_likelihood_K$k[which.max(italy_grid_search_results$inner_grid_search_results$profile_likelihood_K$llk)], italy_grid_search_results$inner_grid_search_results$K_CI[2])
...
last_date <- as.Date('01/08/2020',format = '%d/%m/%y')
state <- 'Italy'
update_italian_data <- mainFunction(state,60, population_list[[state]],just_data = T)
italy_grid_search_results$environment_data <- update_italian_data
...
state <- 'US'
update_usa_data <- mainFunction(state,60, population_list[[state]],just_data = T)
us_grid_search_results$environment_data <- update_usa_data
...
last_date <- as.Date('01/11/2020',format = '%d/%m/%y')
state <- 'Germany'
update_germany_data <- mainFunction(state,60, population_list[[state]],just_data = T)
germany_grid_search_results$environment_data <- update_germany_data
plotTrajectories(germany_grid_search_results$environment_data,germany_trajectories,dates,last_date,state,ylim = c(0,2.0),scale_factor = 1e5)
```
...
state <- 'Israel'
israel_grid_search_results <- mainFunction(state,75, population_list[[state]], cut_off_day = 124)
...
israel_K_grid <- c(israel_grid_search_results$inner_grid_search_results$K_CI[1], 
               israel_grid_search_results$inner_grid_search_results$profile_likelihood_K$k[which.max(israel_grid_search_results$inner_grid_search_results$profile_likelihood_K$llk)], 
...
israel_grid_search_results$environment_data <- israel_grid_search_results_new$environment_data
```
...
plotTrajectories(israel_grid_search_results$environment_data,israel_trajectories,dates,last_date,state,ylim = c(0,1.7),scale_factor = 1e4,starting_day = 75)
```
