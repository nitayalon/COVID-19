    ...
    for(j in 1:calibration_loops)
    {
      if(cpp){
        res <- innerLoop(n,number_of_inner_loops,Beta = beta,Gamma = gamma,
                         Del = del,Alpha = alpha,
                         K = K,x = X_trimmed, vw = VW_trimmed, y = Y_trimmed)
...
beta_mle=beta_hat[single_dim_llk_ind,nrow(beta_hat)]
gamma_mle=gamma_hat[single_dim_llk_ind,nrow(gamma_hat)]
K_one_dim=asympt[single_dim_llk_ind]

two_dim_llk=min(OBJ)
two_dim_llk_ind=which.min(OBJ)
beta_mle_two_dim=beta_hat[two_dim_llk_ind,nrow(beta_hat)]
gamma_mle_two_dim=gamma_hat[two_dim_llk_ind,nrow(gamma_hat)]
...
for(i in (number_of_inner_loops + 1):(number_of_inner_loops * 5 * n))
  {
    # infection rate growth
    x[i] = x[i-1] + beta_mle_two_dim * y[i-1] ^ alpha * max(0, 1 - x[i-1] / K_two_dim) * del
    # removed rate
    vw[i]=vw[i-1] + gamma_mle_two_dim * y[i-1] * del
    # Current ills
    y[i]=max(0,x[i]-vw[i])
  }
for(i in 1:n)
...
lines(VW_trimmed / 1e5,col = 'red')

plot(y[(1:length(y)) %% 100 == 0] / 1e5, col = 'blue', type= 'l')
lines(Y_trimmed / 1e5, col = 'red', type='l')
```
