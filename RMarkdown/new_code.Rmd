...
for(hhh in 1:hhh_upper_limit)
  {
    K = max(X) + hhh * NNN
    asympt[hhh] = K
    beta <- (X[n]-X[1])/ sum(max(1,Y_middle ^ alpha - Y_middle[1] ^ alpha)*(1 - (X_middle-X_middle[1]) / K))
...
```{r llk minimization}
plot(asympt, OBJB, type = 'l')
plot(asympt, OBJ, type = 'l')
```
...
if(min(single_dim_llk_ind,two_dim_llk_ind)>5){
  matrA = matrix(c(rep(1,11), t((single_dim_llk_ind-5):(single_dim_llk_ind+5)), t((single_dim_llk_ind-5):(single_dim_llk_ind+5))^2), nrow = 11);
  cofsA = matrA / OBJ[(single_dim_llk_ind-5):(single_dim_llk_ind+5)]
  wwA=-cofsA[2]/(2*cofsA[3])
  FISHERA=2*cofsA[3]/NNN^2
  KA=max(X)+wwA*NNN
  matrB=matrix(c(rep(1,11), t((two_dim_llk_ind-5):(two_dim_llk_ind+5)), t((two_dim_llk_ind-5):(two_dim_llk_ind+5))^2), nrow = 11);
  cofsB=matrB / OBJB[(two_dim_llk_ind-5):(two_dim_llk_ind+5)]
...
```{r validation plots}
plot(X/ 1e5, col = 'blue' , type = 'b', pch=7, ylim = c(0,2.5))
