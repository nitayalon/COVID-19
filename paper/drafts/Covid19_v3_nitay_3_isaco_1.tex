...
In principle, these first-passage times should follow $\Gamma$ distributions (with some local parameters), the time it takes a Poisson process to reach a given height. Appendix 3 provides a case questioning the validity of this parametric approach in practice.

Diffusion processes assume Gaussian increments, and as such are an inadequate theoretical basis for modelling processes with non-decreasing sample paths, although for sparsely sampled data, this should not be a serious error, and diffusion methods are commonly applied in these situations.

Diffusion processes could provide a basis for identifying the random time increment distribution, as first passage time of Brownian Motion. Appendix 2 quotes from the literature very elegant explicit formulas. However, these formulas are based on reflection principles that hinge in an essential way on the {\em non}-monotonicity of sample paths. Hence, there is no basis for their use in the current setup.

The same criticism applies against the automatic use of transition densities derived by Fokker-Planck equations. But if data is sampled frequently enough so drifts and diffusion coefficients stay essentially constant within observations, these transition densities are in fact regular Gaussian densities.

Returning to the RTT method, the general framework that processes like $X$ and $R$ should be approximately distributed as Levy processes, with roughly i.i.d. increments (the time to increase by $D$ is the sum of $100$ consecutive times to increase by $D \over 100}$) seems reasonable, and the Central Limit Theorem provides an umbrella justification for the working paradigm of Gaussian incremental times with unspecified variance.

Analysis based on RTT has been coupled by analysis based on SDE, to verify that in practice the two lead to similar results.
...
For the application of SDE, the differential functions in (\ref{DEforX})-(\ref{DEforR}) identify the drift terms. Appendix 2 provides an argument introduced in \cite{Bassan} that the closest SDE model to RTT has diffusion term proportional to the drift term. This SDE model will be adopted. Furthermore, consistent with the above arguments, it will be assumed that a day is a long enough interval of time for monotonicity but short enough so drift and diffusion can be considered constant, and the transition densities are Gaussian. For details on stochastic integrals and Fokker-Planck or Kolmogorov forward equations, consult Karatzas and Shreve \cite{Karatzas}.
...
\noindent {\bf The likelihood function induced by the RTT method and SDE methods}. The incremental times $(\Delta_1(m),\Delta_2(m))$ are viewed
