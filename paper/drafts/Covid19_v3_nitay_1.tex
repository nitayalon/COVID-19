...
\bigskip
Before solving equations, a post-preliminary pre-processing validation of the basic model. If \ref{DEforX} is correct then $\log({{\mbox{new cases}} \over {1-X/K}})$ should be linear in $\log(I)$, with slope $\alpha$. "New cases" in a given day is the daily average in the week around the day. $K$ was roughly set as $2 \max(X)$ in the USA and as $1.1 \max(X)$ in Italy and Germany. The figures \ref{fig:linear_prediction_of_log_new_cases} display $\log(\frac{newcases}{1-\frac{X}{K}})$ and the linear predictor.
...
\bigskip
Define the {\em random time trajectory} as starting at $T_1(1)=1, T_2(1)=1$.For $m \ge 2$, let $T_1(m)$ be the smallest ${j \over M}$ for which $x(j) \ge X_j$ and let $T_2(m)$ be the smallest ${j \over M}$ for which $r(j) \ge R_j$. Better yet, let $T_1(m)$ and $T_2(m)$ be the linear interpolants between $j-1$ and $j$ that achieve the values $X_m$ and $R_m$ exactly.

Now solve for $\beta$ and $\gamma$ so that $T_1(n)=T_2(n)=n$. Incremental time has average $1$ in both equations.Define $\Delta_1(m)=T_1(m+1)-T_1(m)$ and $\Delta_2(m)=T_2(m+1)-T_2(m)$, for $m=1, 2, \cdots,m-1$ as the (mean-$1$) increments of the $T_1$ and $T_2$ processes.
...
\noindent {\bf The likelihood function induced by the RTT method}. For simplicity, view the incremental times \linebreak $(\Delta_1(m),\Delta_2(m))$
...
This Appendix concludes with some details on the first passage distribution. If $B_t$ is Brownian motion with drift $\mu>0$ and standard deviation $\sigma$, for every fixed $s \in {\cal R}$, the process $\exp\{s (B_t - \mu t) - s^2 {{\sigma^2} \over 2} t\}$ is a mean-$1$ Martingale. Hence, the first hitting time $\tau$ of $D>0$ satisfies
...
\bibitem{Doeblin} Doeblin, W.

\bibitem{DubSch}  Lester E. Dubins and Gideon Schwarz. (1965)  On continuous martingales.Pro-ceedings of the National Academy of Sciences, 53:913–916, 1965
...
\bibitem{Monroe} Monroe, I. Processes that can be Embedded in Brownian Motion. (1987)
...
\bibitem{Ricciardi} Ricciardi, L. M. (1977). {\bf Diffusion processes and related topics in Biology}. Lecture Notes in Biomathematics, Springer.
...
\bibitem{Skorokhod} Skorokhod, A. V.(1965) Studies in the theory of random  processes. Addison-Wesley. 
