gridSearchMainFunction <- function(national_data, population, starting_day, cut_off_day = NULL, prediction_period = NULL,
  ...
  browser()
  lower_inner_k_grid <- max(unique(outer_grid_parameters$Var2)[which.max(minimal_llk_per_k) - 2], min(k_grid), na.rm = T)
  upper_inner_k_grid <- min(unique(outer_grid_parameters$Var2)[which.max(minimal_llk_per_k) + 2],population,na.rm = T)
  inner_k_grid <- seq(lower_inner_k_grid, upper_inner_k_grid, length.out = 20)
  ...
  COV <- lapply(inner_grid_search_results, function(x){x$COV})
  OBJ <- sapply(inner_grid_search_results, function(x){x$OBJ})
  OBJB <- sapply(inner_grid_search_results, function(x){x$OBJB})
  
  inner_likelihood_matrix <- matrix(OBJ, nrow = length(unique(inner_grid_parameters$Var2)),
