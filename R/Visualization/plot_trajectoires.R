plotTrajectories <- function(nation_data, 
  ...
  max_infection_actual <- max(nation_data$Y / scale_factor)
  ...
  max_infection_actual_index <- which.max(nation_data$Y / scale_factor)
  ...
  lines(seq(min(days),min(days) + (length(nation_data$X) - 1), 'day'), nation_data$X / scale_factor, col = 'blue', type = 'b', pch=7, lwd = 1, cex = 0.5)
  lines(seq(min(days),min(days) + (length(nation_data$X) - 1), 'day'), nation_data$VW / scale_factor, col = 'blue', type = 'b', pch=13, lwd = 1, cex = 0.5)
  lines(seq(min(days),min(days) + (length(nation_data$X) - 1), 'day'), nation_data$Y / scale_factor, col = 'blue', type = 'b', pch=19, lwd = 1, cex = 0.5)
